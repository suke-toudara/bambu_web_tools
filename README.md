...
- **マルチパーツプレート編集**: 追加したパーツはプレート上に自動配置(重ならないよう棚詰め配置)され、クリックで選択してビューア上でハイライト表示。位置・回転・スケールを数値入力で個別に調整でき、「ベッドに接地」ボタンで底面をプレートに合わせ直せます。
